...
echo curl -u \"$GATEWAY_CREDENTIALS\" \"https://$IOT_DOMAIN_HOST/$GATEWAY_ENDPOINT_GATEWAY_STATS_PATH\" -H \'Content-Type: application/json\' -d \"{\\\"payload\\\": {\\\"hasuccess\\\": 24.6,\\\"hafail\\\": 1.4}, \\\"timestamp\\\": \$CURRENT_TS}\"
echo
echo "To send the gateway config test data using https for device $GATEWAY_NAME"
echo 'export CURRENT_TS=`date +%s%N | cut -b1-13`'
echo curl -u \"$GATEWAY_CREDENTIALS\" \"https://$IOT_DOMAIN_HOST/$GATEWAY_ENDPOINT_GATEWAY_CONFIG_PATH\" -H \'Content-Type: application/json\' -d \"{\\\"payload\\\": {\\\"successtimewindow\\\": 10,\\\"failtimewindow\\\": 11}, \\\"timestamp\\\": \$CURRENT_TS}\"
echo
echo "To send gateway stats test data for the configuration using mqttx for device $GATEWAY_NAME"
echo 'export CURRENT_TS=`date +%s%N | cut -b1-13`'
echo mqttx pub -t $GATEWAY_ENDPOINT_GATEWAY_STATS_PATH -ct application/json  -u $GATEWAY_EXTERNAL_KEY -P $GATEWAY_INSTANCE_SECRET  -h $IOT_DOMAIN_HOST -p 8883  -m  \"{\\\"payload\\\": {\\\"hasuccess\\\": 24.6,\\\"hafail\\\": 1.4}, \\\"timestamp\\\": \$CURRENT_TS}\"
echo
echo "To send gateway config test data for the configuration using mqttx for device $GATEWAY_NAME"
echo 'export CURRENT_TS=`date +%s%N | cut -b1-13`'
echo mqttx pub -t $GATEWAY_ENDPOINT_GATEWAY_CONFIG_PATH -ct application/json  -u $GATEWAY_EXTERNAL_KEY -P $GATEWAY_INSTANCE_SECRET  -h $IOT_DOMAIN_HOST -p 8883  -m  \"{\\\"payload\\\": {\\\"successtimewindow\\\": 12,\\\"failtimewindow\\\": 13}, \\\"timestamp\\\": \$CURRENT_TS}\"
echo
