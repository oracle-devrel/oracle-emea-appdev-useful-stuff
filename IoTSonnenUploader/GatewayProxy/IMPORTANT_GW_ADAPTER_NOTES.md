...
If the target is null then the normal envelopeMappings are applied (e.g setting timeObserved if you need to) and it's then handed to the routes part of the gateway adapter. Unclear how the contentRoot is handled in that situation.

The contentRoot is used to specify where in the gateway level input the actual content data exists, for example the gateway may have some meta data and then encapsulate the actual device data in a field (e.g $.payload) and you only want that payload to be handed to the ICD adapters. *THIS ONLY APPLIES TO INDIRECT DEVICES*, i.e. when the target is non null. *FOR THE GW ITSELF the content root is not applied*, so you if there is a payload sub object within the json that contains the time stamp you need to do payload.timestamp- to get the data, even if the contentRoot is .payload (which will remove the outer data and pass the payload on as the root to the ICD adapter ONLY)

So, using a non $ content root at the GW level may mean that normal "root" level data (e.g. timestamps) must be in the new content root when it gets to the ICDs.
