...
@Requires(property = "datareplay.configurationreplay.enabled", value = "true", defaultValue = "false")
public class ConfigurationDataReplay implements Runnable {
	@Getter
	@Setter
	...
	@Property(name = "datareplay.configurationreplay.actuallyupload", defaultValue = "true")
	...
	private SonnenConfiguration nextToSend;
	...
	public void onStartup(StartupEvent event) {
		log.info("Startup event received for configuration replay, input file=" + configurationFileName);
		try {
			reader = new BufferedReader(new FileReader(configurationFileName));
		} catch (IOException e) {
			log.warning("Unable to open reader from file " + configurationFileName + " due to "
		...
		do {
			...
			try {
				...
				log.warning("Unable to read sonnen configuration from line " + lineRead + " due to "
						+ e.getLocalizedMessage());
				return;
			}
			log.info("Skipping forward Just read line " + lineRead + " which is" + nextUploadLine);
			try {
				nextToSend = mapper.readValue(nextUploadLine, SonnenConfiguration.class);
			} catch (IOException e) {
	 ...
	 * 
	 */
	private void sendToMqtt(SonnenConfiguration configurationToSend) {
		if (actuallyUpload) {
			try {
				CompletableFuture<Void> publishResp = mqttSonnenBatteryPublisher
						.publishSonnenConfiguration(configurationToSend);
			...
			log.info("actuallyUpload is false but would have uploaded configuration to mqtt");
		}
	}
		...
		if (stopAfterInputTimestamp) {
			if (nextToSend.getTimestamp().isAfter(stopAfterInputTimestampTimestamp)) {
				log.info("Most recent read input " + nextToSend
						...
						+ stopAfterInputTimestampTimestamp.format(DateTimeFormatter.ISO_DATE_TIME)
						+ "\nStopping configuration upload simulation");
