...
if [ $ATTEMPT = 1 ] ; then
    ...
    if [ "$IS_RUNNING" = true ]; then
        echo "VM reached RUNNING state after $ATTEMPT checks. Waiting 60s for Cloud Agent to initialize..."
        sleep 60
    else
...
SESSION_JSON=`oci bastion session create-managed-ssh  --bastion-id "$BASTION_OCID"     --target-resource-id "$VM_OCID"    --target-os-username "$VM_USER_NAME"  --ssh-public-key-file "$BASTION_PUB_KEY_PATH"  ` # Waits until session is ready to use
