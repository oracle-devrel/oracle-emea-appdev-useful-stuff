...
if [[ -n  "$DIGITAL_TWIN_INSTANCE_OCID" ]]
then
  echo "Located digital twin instance $DIGITAL_TWIN_INSTANCE_DEVICE_NAME, deleting it"
  oci iot digital-twin-instance delete --digital-twin-instance-id $DIGITAL_TWIN_INSTANCE_OCID --force --wait-for-state DELETED
  echo "Deleted digital twin instance $DIGITAL_TWIN_INSTANCE_DEVICE_NAME"
else 
  echo "Can't locate digital twin instance $DIGITAL_TWIN_INSTANCE_DEVICE_NAME"
  exit -3
fi
...
if [[ -n  "$DIGITAL_TWIN_ADAPTER_OCID" ]]
then
  echo "Located digital twin adapter $DIGITAL_TWIN_ADAPTER_NAME, deleting it"
  oci iot digital-twin-adapter delete --digital-twin-adapter-id $DIGITAL_TWIN_ADAPTER_OCID --force --wait-for-state DELETED
  echo "Deleted digital twin adapter $DIGITAL_TWIN_ADAPTER_NAME"
else 
  echo "Can't locate digital twin adapter $DIGITAL_TWIN_ADAPTER_NAME"
  exit -4
fi
...
if [[ -n  "$DIGITAL_TWIN_MODEL_ID" ]]
then
  echo "Located digital twin model $DIGITAL_TWIN_MODEL_NAME, deleting it"
  oci iot digital-twin-model delete --digital-twin-model-id $DIGITAL_TWIN_MODEL_ID --force --wait-for-state DELETED
  echo "Deleted digital twin model $DIGITAL_TWIN_MODEL_NAME"
else 
  echo "Can't locate digital twin model $DIGITAL_TWIN_MODEL_NAME"
  exit -5
fi