...
import com.oracle.demo.timg.iot.iotdbjdbc.messagehandler.RawDataMessageHandlerService;
...
@Singleton
@Log
@Requires(property = "iotdatacache.aq.rawdata.individualreader.enabled", value = "true", defaultValue = "false")
@Requires(property = "iotdatacache.aq.rawdata.individualreader.order")
public class IoTAQRawDataIndividualReader extends IoTAQRawDataCore implements IoTDBClient, Runnable {
	...
	@Inject
	private RawDataMessageHandlerService rawDataMessageHandlerService;

	@Inject
	public IoTAQRawDataIndividualReader(DBConnectionSupplier dbConnectionSupplier,
			...
			@Property(name = "iotdatacache.aq.rawdata.subscribername", defaultValue = "aqclientrawdata") String aqsubscribername,
			@Property(name = "iotdatacache.aq.rawdata.individualreader.readtimeout", defaultValue = "10") @Min(value = 0) int aqReadTimeout,
			@Property(name = "iotdatacache.aq.rawdata.individualreader.order") @Min(value = 0) int order)
	...
	private void readAQMessages() throws SQLException {
		...
		while (!stopped) {
			...
			AQMessage message;
			try {
				message = connection.dequeue(getQueueName(), dequeueOptions, getPayloadType());
