...
@Singleton
@Requires(property = FileWriterProperties.RAW_DATA_FILE_OUTPUT_ENABLED, value = "true", defaultValue = "false")
@Log
public class RawDataFileOutput implements RawDataMessageHandler {
	...
	@Inject
	public RawDataFileOutput(ObjectMapper mapper, DeviceModelInstancesCache deviceModelInstancesCache,
			@Property(name = FileWriterProperties.RAW_DATA_FILE_OUTPUT_ORDER) int order,
			@Property(name = FileWriterProperties.RAW_DATA_FILE_OUTPUT_DURATION, defaultValue = "1h") Duration recordDuration,
			@Property(name = FileWriterProperties.RAW_DATA_FILE_OUTPUT_TARGET_FILE) String outputFile)
			throws IOException {
		this.mapper = mapper;
	...
	public RawData[] processRawData(RawData input) throws Exception {
		...
		RawData[] returnData = new RawData[1];
		returnData[0] = input;
		// if the data
		...
		RawDataFileVersion dataFileVersion = RawDataFileVersion.buildFrom(input, deviceDisplayName);
